...
class PackageParserApi21 extends PackageParser {
    ...
    public void collectCertificates(int flags) throws Exception {
        ...
        Method method = null;
        try {
            method = MethodUtils.getAccessibleMethod(sPackageParserClass, "collectCertificates",
                    mPackage.getClass(), int.class);
            method.invoke(mPackageParser, mPackage, flags);
        } catch (Exception e) {
            Log.d("collectCertificates error :", e.getMessage());
            method = MethodUtils.getAccessibleMethod(sPackageParserClass, "collectCertificates",
                    mPackage.getClass(), Boolean.TYPE);
            method.invoke(mPackageParser, mPackage, flags != 0);
        }
    }
